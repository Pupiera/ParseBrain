...
from speechbrain import hpopt as hp
...
class ASR(sb.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        loss = 0.5 * loss + 0.5 * loss_parsing
        return loss
    ...
    def compute_objectives_parsing(self, predictions, batch, stage, predicted_words=None):
        ...
        loss_transducer_pos = self.hparams.transducer_cost_pos(
            predictions["logits_transducer_pos"], pos, predictions["wav_lens"], token_lens
        )
        loss_transducer_gov = self.hparams.transducer_cost_gov(
            predictions["logits_transducer_gov"], gov, predictions["wav_lens"], token_lens
        )
        loss_transducer_dep = self.hparams.transducer_cost_dep(
            predictions["logits_transducer_dep"], dep, predictions["wav_lens"], token_lens
        )
        ...
        loss = self.hparams.pos_weight * loss_transducer_pos + self.hparams.gov_weight *loss_transducer_gov + self.hparams.dep_weight * loss_transducer_dep
        if hasattr(self.hparams, "dec_pos_lin"):
...
if __name__ == "__main__":
    # CLI:
 with hp.hyperparameter_optimization(objective_key="LAS") as hp_ctx: # <-- Initialize the context

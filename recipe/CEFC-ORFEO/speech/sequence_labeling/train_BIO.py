...
class ASR(sb.core.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        #logits_bio = self.modules.bio_lin(x)
        bio_scores = self.modules.bio_crf(x)
        ...
        result = {"p_ctc": p_ctc, "bio_scores": bio_scores, "wav_lens": wav_lens, "length": length}
        if sb.Stage.TRAIN != stage:
            result["bio_decoded"] = self.hparams.bio_decoder.decode(bio_scores, length)
        if self.is_training_syntax:
            ...
            map_frameToWord = self.colaspe_bio(result["bio_decoded"])
            input_dep, seq_len = self._create_inputDep(x, bio)
    ...
    def colapse_bio(self, p_bio):
        ...
        for bio in batch_bio:
            ...
            for b in bio:
                ...
                if b == 2 and inside:
                    loc_map.append(index)
                #Inside (and begin is not before)
                else:
                    loc_amp.append(0)
            map_frame.append(torch.tensor(loc_map))
        return torch.pad(mapFrameToWord)
    ...
    def compute_objective_bio(self, predictions, batch, stage):
        start_words = batch.start_words.data
        end_words = batch.end_words.data
        #p_bio = predictions['p_bio']

        bio_supervision = self.compute_bio(predictions["bio_scores"], predictions["length"], start_words, end_words)

        #print(bio_supervision)
        #loss = self.hparams.bio_loss(p_bio, bio_supervision, predictions["wav_lens"])
        loss = self.hparams.bio_loss(predictions["bio_scores"], bio_supervision, predictions["length"])

        if sb.Stage.TRAIN != stage:
            bio_decoded = predictions["bio_decoded"]
            #self.bio_metrics.append(batch.id, bio_decoded, bio_supervision)
            self.bio_metrics.update(preds = bio_decoded, target = bio_supervision)
            #remove padding to supervision and to decoded
            #self.bio_gold.append(bio_supervision)
            #self.bio_preds.append(bio_decoded)
        return loss
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        loss_bio = self.compute_objective_bio(predictions, batch, stage)
        #print(loss_bio)
        loss += self.hparams.BIO_weight * loss_bio
        if self.is_training_syntax:
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.TRAIN:
          ...
          # stage_stats["F1_BIO"] = self.bio_metrics.summarize()["F-score"]
            ...
            fig_, ax_ = self.bio_metrics.plot()
            fig.set_title("BIO F1 score")
            fig.savefig("F1_BIO.png")
...
class f1_score_tolerant:
    '''
    Class to compute f1 score of multiclass sequence predictions with a tolerance (if the correct one is predicted in a range around it, maybe only for specific labels ?)
    '''
    def __init__(self, f1_type='macro', tolerance=0):
        self.f1_type = f1_type 
        self.tolerance = tolerance

    def update(self, predictions, supervisions):
        pass
...
if __name__ == "__main__":
    ...
    asr_brain.hparams.bio_loss.set_tag_map(BIO_dict)
    asr_brain.hparams.bio_decoder.set_tag_map(BIO_dict)
    # Training
    try:
        asr_brain.fit(

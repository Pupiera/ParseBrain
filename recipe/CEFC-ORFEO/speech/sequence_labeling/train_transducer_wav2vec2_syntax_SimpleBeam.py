...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            best_hyps = self.hparams.beam_searcher(x)
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            pred_asr = [x['-asr'][0] for x in predictions["predicted_tokens"]]
            ...
            for i in range(batch_size):
                ...
                if len_pred_word == 1 and predicted_words[i][0] == '':
                    len_pred_word = 0
                len_pred = min([len_pred_word, len(predictions["predicted_tokens"][i]['-pos'][0]),
                    len(predictions["predicted_tokens"][i]['-gov'][0]), len(predictions["predicted_tokens"][i]['-dep'][0])])
                #predicted_words[i] = " ".join(split_words[:len_pred])
                predicted_words[i] = predicted_words[i][:len_pred]
                predictions["predicted_tokens"][i]['-pos'][0] = predictions["predicted_tokens"][i]['-pos'][0][:len_pred]
                predictions["predicted_tokens"][i]['-gov'][0] = predictions["predicted_tokens"][i]['-gov'][0][:len_pred]
                predictions["predicted_tokens"][i]['-dep'][0] = predictions["predicted_tokens"][i]['-dep'][0][:len_pred]
    ...
    def compute_objectives_parsing(self, predictions, batch, stage, predicted_words=None):
        ...
        if stage != sb.Stage.TRAIN:
            ids = batch.id
            pred_gov = [x['-gov'][0] for x in predictions['predicted_tokens']]
            pred_dep = [x['-dep'][0] for x in predictions['predicted_tokens']]
            pred_pos = [x['-pos'][0] for x in predictions['predicted_tokens']]

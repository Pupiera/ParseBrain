...
class Parser(sb.core.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        try:
            assert pos.shape[-1] == head.shape[-1]
            assert pos.shape[-1] == dep.shape[-1]
            ...
            print(f"seq len : {predictions['seq_len']}")
            print(f"pos : {pos}, {pos.shape}")
            print(f"gov : {head}, {head.shape}")
            print(f"dep : {dep}, {dep.shape}")
            print(f"pred pos : {predictions['p_posLabel'].shape}")
            print(f"pred gov : {predictions['p_govLabel'].shape}")
            print(f"pred dep : {predictions['p_depLabel'].shape}")
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            metrics_dict = self.hparams.evaluator.evaluateCoNLLU(goldPath_CoNLLU, st, gold_segmentation=True)
...
def dataio_prepare(hparams, tokenizer):
    ...
    def syntax_pipeline(wrd, pos, head, dep):
        ...
        """
        assert len(pos) == len(head)
        assert len(pos) == len(dep)
        try:
            pos_list = torch.LongTensor([label_alphabet[2].get(p) for p in pos])
        except TypeError:
            print([label_alphabet[2].get(p) for p in pos])
        yield pos_list
        try:
        ...
        yield depDep2Label
        assert pos_list.shape == govDep2label.shape
        assert pos_list.shape == depDep2Label.shape

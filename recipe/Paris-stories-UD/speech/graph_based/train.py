    ...
    def compute_forward(self, batch, stage):
        ...
        if self.is_training_syntax:
            sequence, mapFrameToWord = ctc_greedy_decode(
                p_ctc, wav_lens, blank_id=self.hparams.blank_index, space_id = 11
            )
    ...
    def compute_objectives_syntax(
        ...
        try:
            ...
            try:
                ...
                print(f"gold_pos {gold_pos}")
                print(p_posLabel.shape)
                print(f"gold_head {gold_head}")
                print(p_govLabel.shape)
                print(f"gold_dep {gold_dep}")
                print(predicted_labels_scores.shape)
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            if self.is_training_syntax:
                ...
                dict_wer_details = {}
                for wer_details in self.stage_wer_details:
                    dict_wer_details[wer_details["key"]] = wer_details
                ...
                self.stage_wer_details = []
                for o in order:
                    self.stage_wer_details.append(dict_wer_details[o])
...
def dataio_prepare(hparams, tokenizer):
    ...
    def syntax_pipeline(pos, head, dep):
        ...
        #print(f"dep_token.shape {dep_token.shape}")
        yield dep_token
    ...
    with open(path, "r", encoding="utf-8") as conllu_file:
        for line in conllu_file:
            ...
            if line =="\n":
                continue
            fields = line.split("\t")
            pos = fields[POS].upper().strip()
            if pos not in pos_dict.keys():
                pos_dict[pos] = len(pos_dict) 

            dep = fields[DEP].upper().strip()
            if dep not in dep_label_dict.keys():
                dep_label_dict[dep] = len(dep_label_dict)
    pos_dict["DELETION"] = len(pos_dict)
    pos_dict["INSERTION"] = len(pos_dict)
    pos_dict["ROOT"] = len(pos_dict)

    dep_label_dict["DELETION"] = len(dep_label_dict)
    dep_label_dict["INSERTION"] = len(dep_label_dict)

    return pos_dict, dep_label_dict

            



pos_dict, dep_label_dict = get_dict_from_conllu("/home/getalp/pupiera/data/UD/UD_French-ParisStories/conllu_NO_PUNCT_NEW_LABEL/fr_parisstories-ud-all_NO_PUNCT_NEW_LABEL_TIMESTAMP.conllu")
print(pos_dict)
print(dep_label_dict)
'''
dep_label_dict = {
    "periph": 0,
    "subj": 1,
...
if __name__ == "__main__":
    ...
    from PS_prepare import prepare_PS #noqa

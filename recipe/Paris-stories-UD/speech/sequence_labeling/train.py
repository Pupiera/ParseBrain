...
class ASR(sb.core.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        # if true we use a specific layer for the syntax parsing and the last one for speech recognition
        if hasattr(self.hparams, "syntax_layers"):
            print("using specific layer")
        ...
        if self.is_training_syntax:
            ...
            print(sequence)
            print("frame level")
            print(torch.argmax(p_ctc, dim=-1))
            print(p_ctc.shape)
            print(mapFrameToWord)
            '''
            if hasattr(self.hparams, "syntax_layers"):
                print("using specific layer")
    ...
    def compute_objectives_syntax(
        ...
        try:
            ...
            #print(f" pred_words : {predicted_words}")
            ...
            try:
                ...
                print(f"argmax pred pos  {torch.argmax(p_posLabel, dim =-1)}")
                '''
    ...
    def fit_batch(self, batch):
        ...
        if self.auto_mix_prec:
            ...
            #plot_grad_flow_v2(self.modules.named_parameters())
            if self.check_gradients(loss):
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            if self.is_training_syntax:
                ...
                # write accumulated wer_details in the same order as the conll file.
                dict_wer_details = {}
...
def dataio_prepare(hparams, tokenizer):
    ...
    def dep2label_pipeline(wrd, poss, gov, dep):
        ...
        try:
            #print([fl.upper().split("\t")[-1].split("{}")[0] for fl in fullLabel])
            govDep2label = torch.LongTensor(
                [
                    label_alphabet[0].get(fl.upper().split("\t")[-1].split("{}")[0])
                    for fl in fullLabel
                ]
            )
            #print(govDep2label)
        ...
        try:
            depDep2Label = torch.LongTensor(
                [label_alphabet[1].get(fl.upper().split("{}")[1]) for fl in fullLabel]
            )
            yield depDep2Label
        except TypeError as e:
            print(wrd)
            print([fl.split("{}")[1] for fl in fullLabel])
            print([label_alphabet[1].get(fl.upper().split("{}")[1]) for fl in fullLabel])
            raise e

...
def create_csv(
    ...
    csv_lines = [["ID", "duration", "wav", "wrd", "pos", "gov", "dep", "begin_time", "end_time"]]
    ...
    for line in tzip(loaded_csv):
        ...
        try:
            sent_id_split = re.split(split_pattern, file)
            if len(sent_id_split) == 4:
                base_name = "_".join(sent_id_split[0:3])
            elif len(sent_id_split) == 5:
                if "_bis" in file:
                    base_name = "_".join(sent_id_split[0:3])
                else:
                    base_name = "_".join(sent_id_split[0:4])
            folder = base_name
        ...
        words = re.sub(
            "[^’'A-Za-z0-9À-ÖØ-öø-ÿЀ-ӿéæœâçèàû~î\-]+", " ", words
        ).upper()
        words = words.replace("AUJOURD'HUI", "AUJOURDHUI")
        words = words.replace("QUELQU'UN", "QUELQUUN")
        words = words.replace("RUPAUL'S", "RUPAULS")
        words = words.replace("O'TAKEY", "OTAKEY")
        words = words.replace("'", " ")
        words = words.replace("’", " ")
        words = words.replace("~","")
        ...
        begin_time = splited[6]
        end_time = splited[7].replace("\n", "")
        assert len(pos.split()) == len(gov.split()), f" {s_id}" 
        assert len(pos.split()) == len(dep.split()), f" {s_id}"
        if "C EST-À-DIRE" in words and len(words.split()) > len(pos.split()):
            words = words.replace("C EST-À-DIRE", "CEST-À-DIRE")
        assert len(pos.split()) == len(words.split()),f" {s_id} {words}"
        # Composition of the csv_line
        csv_line = [s_id, str(duration), wav_path, str(words), pos, gov, dep, begin_time, end_time]
...
def check_PS_file(data_folder):
    ...
    """
    files_str = "/ParisStories_2019_concoursEquitation/"
    ...
    if not os.path.exists(data_folder + files_str):
        ...
        err_msg = (
            ...
            "the Paris_stories dataset)" % (data_folder + files_str)
        )
...
if __name__ == "__main__":
    ...
    user_folder = "/home/getalp/pupiera/data/UD/UD_French-ParisStories"
    data_folder = user_folder +"/wavs/splited_wav"
    save_folder = "ParisStories_save"
    train_tsv_file = (
        user_folder + "/train.tsv"
    )
    dev_tsv_file = (
        user_folder + "/valid.tsv"
    )
    test_tsv_file = (
        user_folder + "/test.tsv"
    )

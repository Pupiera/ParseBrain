    ...
    if isinstance(string_pred, list):
        ...
        for i, v in enumerate(string_pred):
            if i == 0 or v != string_pred[i - 1]:  # filter repetition
                string_out.append(v)
                if string_pred[i] == 1:  # filter on space ==1
                    previous_space = True
                    cpt += 1
                ...
                if string_pred[i] == 1:
                    ...
                    string_pred[i] == 0 and previous_space == True
                ):  # deal with case of space followed by blanck.
                    map_frame_to_ASR.append(0)
                else:
                    map_frame_to_ASR.append(cpt)
        ...
        # remove following 1. Should remove empty string
        string_out = [
            e
            for i, e in enumerate(string_out)
            if i == 0 or not (e == 1 and string_out[i - 1] == 1)
        ]  
        # print(string_out)
    else:
...
def ctc_greedy_decode(probabilities, seq_lens, blank_id=-1):
    ...
    for seq, seq_len in zip(probabilities, seq_lens):
        ...
        out, map = filter_ctc_output(predictions.tolist(), blank_id=blank_id)

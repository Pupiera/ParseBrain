...
class S2SBaseSearcherSyntax(torch.nn.Module):
    ...
    def __init__(
        ...
        super(S2SBaseSearcherSyntax, self).__init__()
        self.bos_index = bos_index
        self.eos_index = eos_index
        self.min_decode_ratio = min_decode_ratio
        self.max_decode_ratio = max_decode_ratio
        self.final_subtoken = None

    def forward(self, syntax_input, enc_states, wav_len, hidden_state=None):
    ...
    def forward_step(self, inp_tokens, syntax_emb, memory, enc_states, enc_lens):
    ...
    def set_mem(self, batch_size, hidden_states, device):
        """This method should implement the resetting of
        memory variables for the seq2seq model.
        E.g., initializing zero vector as initial hidden states.
    ...
    def update_index_n_word(self, index_n_word, inp_tokens):
        """

        @param index_n_word: list of shape [batch*beam] storing the current index of word for the beam
        @param inp_tokens: tensor of shape [batch*beam] contains last generated tokens
        @return: index_n_word incremented where inp_tokens is the end of full tokens (ie : not a non finals subtokens)
        """
        for i in range(len(index_n_word)):
            if inp_tokens[i] in self.final_subtoken:
                index_n_word[i] += 1
        return index_n_word

    def get_syntax_emb(self, syntax_input, index_n_word):
        """

        @param syntax_input: tensor of shape [batch*beam, seq_len, dim]
            store the syntax embedding for each word that will be generated.
        @param index_n_word:  list of shape [batch*beam], contains the current word index for the beam.
        @return : a tensor of shape [batch*beam, dim] with the relevant syntax embedding for the current word (ie : the
        value in index_n_word)
        """
        gathered_emb = []
        seq_len = syntax_input.size(1)
        for i in range(syntax_input.size(0)):
            word_index = index_n_word[i]
            if word_index >= seq_len:
                word_index = seq_len - 1
            gathered_emb.append(syntax_input[i, word_index, :])
        gathered_emb = torch.stack(gathered_emb)
        return gathered_emb

    def set_list_final_subtoken(self, final_subtoken):
        self.final_subtoken = final_subtoken


############################################### GREEDY #####################################################


class S2SGreedySearcherSyntax(S2SBaseSearcherSyntax):
    """This class implements the general forward-pass of
    greedy decoding approach. See also S2SBaseSearcher().
    """

    def forward(self, syntax_input, enc_states, wav_len, hidden_states=None):
        """This method performs a greedy search.
        Arguments
        ---------
        syntax_input: torch.tensor
            The syntactic information used to influence the decpding of the seq2seq model.
        enc_states : torch.Tensor
            The precomputed encoder states to be used when decoding.
            (ex. the encoded speech representation to be attended).
        wav_len : torch.Tensor
        ...
        if hidden_states is not None:
            memory = self.set_memory(batch_size, hidden_states, device=device)
        else:
        ...
        index_n_word = [0 for _ in range(batch_size)]
        for t in range(max_decode_steps):
            synt_emb = self.get_syntax_emb(syntax_input, index_n_word)
            log_probs, memory, _ = self.forward_step(
                inp_tokens, synt_emb, memory, enc_states, enc_lens
            )
            log_probs_lst.append(log_probs)
            inp_tokens = log_probs.argmax(dim=-1)
            index_n_word = self.update_index_n_word(index_n_word, inp_tokens)
...
class S2SRNNGreedySearcherSyntax(S2SGreedySearcherSyntax):
    """
    ...
    **kwargs
    ...
    ...     "gru", "content", 3, 3, 1, enc_dim=7, input_size=6
    ... )
    >>> lin = sb.nnet.linear.Linear(n_neurons=5, input_size=3)
    >>> searcher = S2SRNNGreedySearcherSyntax(
    ...     embedding=emb,
    ...     decoder=dec,
    ...     linear=lin,
    ...     bos_index=4,
    ...
    >>> syntax_emb = torch.rand([2,2,3])
    >>> hyps, scores = searcher(syntax_emb, enc, wav_len)
    """

    def __init__(self, embedding, decoder, linear, **kwargs):
        super(S2SRNNGreedySearcherSyntax, self).__init__(**kwargs)
        self.emb = embedding
        self.dec = decoder
        self.fc = linear
    ...
    def forward_step(self, inp_tokens, syntax_emb, memory, enc_states, enc_lens):
        ...
        e = torch.cat((e, syntax_emb), dim=-1)
...
class S2SBeamSearcherSyntax(S2SBaseSearcherSyntax):
    ...
    def __init__(
        ...
        super(S2SBeamSearcherSyntax, self).__init__(
            bos_index,
            eos_index,
            min_decode_ratio,
            max_decode_ratio,
    ...
    def forward(self, syntax_input, enc_states, wav_len):  # noqa: C901
        ...
        # inflate the syntax_input by beam_size times.
        syntax_input = inflate_tensor(syntax_input, times=self.beam_size, dim=0)
        ...
        index_n_word = [0 for _ in range(batch_size * self.beam_size)]
        ...
        for t in range(max_decode_steps):
            ...
            # define function from : syntax_input of shape [batch, seq_len, dim] (maybe [batch*beam seq_len, dim] is better
            # and from index_n_word of shape [batch*beam] which contains the the current indice of the word in the beam
            # get all the relevant syntax embedding (at index_n_word) for each beam
            syntax_emb = self.get_syntax_emb(syntax_input, index_n_word)

            log_probs, memory, attn = self.forward_step(
                inp_tokens, syntax_emb, memory, enc_states, enc_lens
            )
...
class S2SRNNBeamSearcherSyntax(S2SBeamSearcherSyntax):
    """
    ...
    **kwargs
    ...
    >>> emb = torch.nn.Embedding(5, 3)
    >>> dec = sb.nnet.RNN.AttentionalRNNDecoder(
    ...     rnn_type="gru", attn_type="content", hidden_size=3, attn_dim=3, num_layers=1, enc_dim=7, input_size=6
    ... )
    >>> lin = sb.nnet.linear.Linear(n_neurons=5, input_size=3)
    >>> ctc_lin = sb.nnet.linear.Linear(n_neurons=5, input_size=7)
    >>> searcher = S2SRNNBeamSearcherSyntax(
    ...     embedding=emb,
    ...     decoder=dec,
    ...     linear=lin,
    ...     ctc_linear=ctc_lin,
    ...
    ...     beam_size=2,
    ... )
    >>> searcher.set_list_final_subtoken([0,1,2,3,4])
    >>> enc = torch.rand([2, 6, 7])
    >>> wav_len = torch.rand([2])
    >>> syntax_emb = torch.rand([2,2,3])
    >>> hyps, scores = searcher(syntax_emb, enc, wav_len)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(S2SRNNBeamSearcherSyntax, self).__init__(**kwargs)
        self.emb = embedding
        self.dec = decoder
        self.fc = linear
...
def mask_by_condition(tensor, cond, fill_value):
    ...
    tensor([[1., 2., 0.],
            [4., 0., 0.]])
    """
    tensor = torch.where(cond, tensor, torch.Tensor([fill_value]).to(tensor.device))
    return tensor

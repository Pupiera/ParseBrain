from typing import List, Tuple, cast, NamedTuple

import numpy as np
import torch

"""
This file is from HOPSparser : https://raw.githubusercontent.com/hopsparser/hopsparser/main/hopsparser/mst.py
"""
...
def score_to_tree(
    ...
    form: list[str],
    reverse_pos_dict,
    reverse_dep_label_dict,
    greedy: bool = False,
    root_in_form: bool = True,
):
    """
    This method take one sentence at a time.
    Create a tree from the probability predicted by the model using a maximum spanning tree method on the graph.
    @param arc_scores: Tensor containing the log prob scores for the arc (ouput of NN)
    @param lab_scores: Tensor containing the log prob scores for the dependency relation: DEPREL (ouput of NN)
    @param tagger_scores: Tensor containing the log prob scores for the Part of speech tagging (ouput of NN)
    @param sent_id: The name of the sentence
    @param greedy: A boolean to choose the way to create the tree. Note that if greedy is true, the output is not necessarily a tree (cycle are possible)
    @param form: The form of each word in the sentence
    @return:
    ...
    pos_tags = [reverse_pos_dict[idx] for idx in tag_idxes[1:].tolist()]
    # `[1:]` to ignore the root node's head
    heads = mst_heads[1:].tolist()
    # `[1:]` to ignore the root node's deprel
    deprels = [reverse_dep_label_dict[lbl] for lbl in mst_labels[1:].tolist()]
    # `[1:]` to ignore the root form "root"
    if root_in_form:
        form = form[1:]
    edges = [
        Edge(head_idx, lbl, dep_idx)

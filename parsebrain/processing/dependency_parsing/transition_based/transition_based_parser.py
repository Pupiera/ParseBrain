    ...
    def parse(self, config, stage, gold_config=None):
        ...
        while any([not self._is_terminal(conf) for conf in config]):
            ...
            if stage == sb.Stage.TRAIN and gold_config is not None:
                dynamic_oracle_decision = self._get_oracle_move_from_config_tree(
                    config, gold_config, dynamic_oracle_decision
                )
            # batched find best decision
            decision_taken = self._get_best_valid_decision(decision_score, config)
            """
            if stage == sb.Stage.TRAIN and gold_config is not None:
                # output the correct label if the arc exist, output either :
                # -1 or the predicted label if arc does not exist;
                dynamic_oracle_label = self._get_oracle_label_from_config_tree(config, gold_config,
                                                                           decision_taken)
            """

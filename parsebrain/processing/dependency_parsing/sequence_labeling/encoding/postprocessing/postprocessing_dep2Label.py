...
"""
This class is from DEP2LABEL (postprocessing_dep2Label.py, part for writing on conllu file was removed since already supported)
"""

# todo: Adapt this to be generic and not super specific to dep2label.


class LabelPostProcessor_dep2Label:
    def tag_BOS(self, task, words_with_labels):
        bos = "-BOS-"
        if task == "1-task":
            label = str(bos)
        elif task == "2-task-combined":
            label = str(bos + "@" + bos + "{}" + bos)
        elif task == "2-task":
            label = str(bos + "{}" + bos)
        elif task == "3-task":
            label = str(bos + "{}" + bos + "{}" + bos)

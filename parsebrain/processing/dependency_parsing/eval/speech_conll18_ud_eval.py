...
# Todo: Update alignment tools to use speechbrain alignement
...
class SmglToken:
    ...
    def __str__(self):
        return f"type : {self.type}, ref : {self.ref}, hyp : {self.hyp}"
...
def load_conllu(file):
    ...
    while True:
        ...
        if not columns[FORM]:
            raise UDError(f"There is an empty FORM in the CoNLL-U file {columns}")
...
class Evaluator:
    ...
    def _is_token_in_sent(self, sent_spans, token_spans):
        return (
            sent_spans.start <= token_spans.start and sent_spans.end >= token_spans.end
        )
    ...
    def spans_score_unaligned_tokens(
        self,
        sentences_gold_spans,
        sentences_system_spans,
        token_gold_spans,
        token_system_spans,
        gold_characters,
        system_characters,
        ...
        for s_g_spans, s_s_spans in zip(sentences_gold_spans, sentences_system_spans):
            ...
            gold_tokens, index_gold = self.get_token_in_sent(
                s_g_spans, token_gold_spans, index_gold
            )
            sys_tokens, index_sys = self.get_token_in_sent(
                s_s_spans, token_system_spans, index_sys
            )
            if len(gold_tokens) != len(
                sys_tokens
            ):  # Need to realign token because of segmentation error in system tokens
                    ...
                    max_sim = max(
                        [
                            self.string_similarity(
                                "".join(gold_characters[t_g.start : t_g.end]),
        ...
        return (
            Score(len(sentences_gold_spans), len(sentences_system_spans), correct_sent),
    ...
    def align_words(self, gold_words, system_words):
        ...
        while gi < len(gold_words) and si < len(system_words):
            if gold_words[gi].is_multiword or system_words[si].is_multiword:
                ...
                gs, ss, gi, si = self.find_multiword_span(
    ...
    def align_words_ASR(self, gold_words, system_words, smgl_alignment):
        ...
        for i in range(len(smgl_alignment)):
            ...
            if smgl_token.type == "D":
                ...
                try:
                    dummy_UD = [
                        int(system_words[index].columns[0]) + 1,
                        "DUMMYS",
                        "_",
                        "DUMMYS",
                        "_",
    ...
    def evaluate(self):
        ...
        return {
            ...
            "CLAS": self.alignment_score(
                alignment,
                lambda w, ga: (ga(w.parent), w.columns[DEPREL]),
                filter_fn=lambda w: w.is_content_deprel,
            ),
            "MLAS": self.alignment_score(
                alignment,
                lambda w, ga: (
                    ga(w.parent),
                    w.columns[DEPREL],
...
def load_speechbrain_file(path):
    _file = open(
        path, mode="r", **({"encoding": "utf-8"} if sys.version_info >= (3, 0) else {})
    )
    return load_speechbrain_alignment(_file)


def load_speechbrain_alignment(file):
    """
    Load format from speechbrain wer_details
    This function heavily rely on the print format of the print_alignments function of speechbrain.

    >>> import speechbrain as sb
    >>> from speechbrain.utils.edit_distance import wer_details_for_batch
    >>> from speechbrain.dataio.wer import print_alignments
    >>> sent_ids = ["test_1","test_2"]
    >>> target_words = [["HELLO", "HOW", "ARE" ,"YOU"], ["I", "LOVE", "PARSING", "AND", "YOU"]]
    >>> predicted_words = [["HELLO", "ARE", "U", "GIBERISH"], ["I", "LOVE", "SPEECH", "ANDD"]]
    ...
    ...        )
    >>> file_print = "./test_load_speechbrain"
    >>> with open(file_print, "w", encoding="utf-8") as f:
    ...     print_alignments(wer_details, file=f, print_header = True)
    >>> f =  open(file_print, mode="r", **({"encoding": "utf-8"} if sys.version_info >= (3, 0) else {}))
    >>> res = load_speechbrain_alignment(f)
    >>> res = [str(r) for r in res]
    >>> print(res)
    ['type :   C   , ref : HELLO , hyp : HELLO ', 'type :    D   , ref :   HOW  , hyp :  <eps> ', 'type :   C  , ref :  ARE , hyp :  ARE ', 'type :   S  , ref :  YOU , hyp :   U  ', 'type :     I    , ref :   <eps>  , hyp :  GIBERISH', 'type : C , ref : I , hyp : I ', 'type :   C   , ref :  LOVE , hyp :  LOVE ', 'type :     S    , ref :  PARSING , hyp :   SPEECH ', 'type :   S   , ref :  AND  , hyp :  ANDD ', 'type :    D  , ref :   YOU , hyp :  <eps>']
...
def main():
    ...
    parser.add_argument(
        "alignment_file",
        type=str,
        help="Path of the output alignment_file with speechbrain format (from wer_details).",
    )
    parser.add_argument(
